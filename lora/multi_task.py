...
class MultiTaskModel(nn.Module):
    ...
    def __init__(
        ...
        self.activate_downstream = functools.partial(self._apply_lora, mode="add")
        self.deactivate_downstream = functools.partial(self._apply_lora, mode="subtract")
    ...
    def apply_downstream(self, downstream_checkpoint):
        ...
        self.activate_downstream(lora_parameters, lora_scalings)
        
        try:
            downstream_model = self.pretrained_model
            yield downstream_model
        finally:
            self.deactivate_downstream(lora_parameters, lora_scalings)
            self.prompt_format = ""
